...
export default [
  ...
  },
  {
    ...
    "lo": "1.2.3",
    "level": "K2",
    "text": "장애인 사용자를 고려하지 않은 사용자 인터페이스를 설계했다. 설계자가 피곤했기 때문이었을 것으로 보인다. 개발자는 설계에 따라 사용자 인터페이스를 구현했지만, 엄청난 시간적 압박 때문에 보너스 계산 코드에 적절한 예외 처리를 포함시키지 않았다. 배포 후 장애인 사용자 일부가 인터페이스에 대해 불만을 제기했고, 관련 규제 당국이 회사에 벌금을 부과했다. 보너스 계산이 때때로 부정확하다는 사실은 아직 아무도 알아채지 못했다. 다음 중 올바른 설명은?",
    "options": [
  ...
  },
  {
    ...
    "lo": "1.3.1",
    "level": "K2",
    "text": "테스터가 테스트 케이스를 작성하고 테스트를 실행하기 위해 테스트 컨디션을 사용하고 있다. 테스트 컨디션은 동일하게 유지되지만 테스트 케이스는 매번 달라진다. 다음 중 이런 테스트 케이스의 달라짐을 통해 다루고 있는 ‘테스팅 원리’는?",
    "options": [
      "테스트 효과는 줄어든다",
      "결함 부재는 궤변이다",
  ...
  },
  {
    ...
    "options": [
      ...
      "회고는 애자일 팀 구성원들이 관리자나 고객에 대한 우려사항을 비난받지 않는 환경에서 표명할 수 있는 기회이다",
      "회고는 애자일 팀에게 다음 반복주기의 계획 및 기술적 결정 사항에 초점을 맞춰 논의할 수 있는 자리를 제공한다"
    ],
    "answer": [
      1
    ],
  ...
  },
  {
    ...
    "options": [
      "개발자가 환전 시스템을 바꾸기 어렵다고 보고했고 테스터는 이것이 사실인지 확인하기로 했다",
      "환전 시스템의 환불 기능이 고객에게 정확한 금액을 지급하지 않는 경우가 있어 제거했다",
      "애자일 팀이 환전 시스템에 새로운 고객 포인트 기능을 추가하는 사용자 스토리의 개발을 시작했다",
  ...
  },
  {
    ...
    "lo": "3.2.3",
    "level": "K1",
    "text": "다음 중 리뷰 회의가 효과적으로 진행되고 회의에 참석한 모든 사람이 자신의 의견을 자유롭게 표현할 수 있도록 하는 책임을 가진 리뷰 프로세스 참가자는?",
    "options": [
      "관리자",
      "중재자",
      "위원장",
  ...
  },
  {
    ...
    "lo": "4.1.1",
    "level": "K2",
    "text": "전자 상거래 웹 애플리케이션의 시스템 테스팅을 수행하고 있으며, 다음과 같은 요구사항이 제공되었다: REQ 05-017. 총 구매 비용이 $100를 초과하는 경우 고객은 이후 구매 시 5% 할인을 받는다. $100를 초과하지 않는 고객에게는 할인이 제공되지 않는다. 다음 중 이 요구사항을 기반으로 테스트 케이스를 설계하는 데 가장 적합한 테스트 기법은?",
    "options": [
      "화이트박스 테스트 기법",
      "블랙박스 테스트 기법",
      "경험 기반 테스트 기법",
  ...
  },
  {
    ...
    "lo": "4.2.1",
    "level": "K3",
    "text": "영화 예매 시스템은 고객의 출생 연도(BY)와 현재 연도(CY)를 기준으로 다음과 같이 할인 유형을 계산한다: D를 CY와 BY의 차, 즉 D=CY-BY라고 하자. • D < 0              “생년월일은 현재 연도보다 클 수 없습니다” 오류 메시지 • 0 ≤ D < 18         학생 할인 적용 • 18 ≤ D < 65       할인 없음 • D ≥ 65               경로 할인 적용 테스트 스위트에 이미 두(2)개의 테스트 케이스가 포함되어 있다. • BY = 1990, CY = 2020,       기대 결과: 할인 없음 • BY = 2030, CY = 2029,       기대 결과: 오류메시지 출력 다음 중 할인 유형에 대한 100% 유효 동등분할 커버리지 달성을 위해 추가해야 하는 테스트 데이터 세트는? (두 개 선택)",
    "options": [
      "BY = 2001, CY = 2065",
      "BY = 1900, CY = 1965",
      "BY = 1965, CY = 1900",
  ...
  },
  {
    ...
    "lo": "4.2.2",
    "level": "K3",
    "text": "당신은 원예용 냉장 시설의 온도 제어 시스템을 테스트하고 있다. 시스템은 온도(정수 단위)를 입력으로 받는다. 온도가 0도 이상 2도 이하면 시스템은 “적정 온도”라는 메시지를 표시한다. 온도가 그보다 낮은 경우 시스템은 “온도가 너무 낮음” 이라는 메시지를 표시하고, 온도가 높은 경우 “온도가 너무 높음” 이라는 메시지를 표시한다. 다음 중 두 개 선택(2-value) 경계값 분석으로 가장 높은 경계값 커버리지를 제공하는 테스트 입력 세트는 무엇인가?",
    "options": [
      "–1, 3",
      "0, 2",
      "–1, 0, 2, 3",
  ...
  },
  {
    ...
    "lo": "4.2.3",
    "level": "K3",
    "text": "다음 결정 테이블을 기반으로 테스트 케이스를 설계하고 있다: 지금까지 아래와 같은 테스트 케이스를 설계했다: • TC1: 19세, 경험이 없는 미등록 남성; 기대 결과: 분류 A • TC2: 65세, 5년 경력의 미등록 여성; 기대 결과: 분류 B • TC3: 66세, 경험이 없는 등록 남성; 기대 결과: 분류 C • TC4: 65세, 4년 경력의 등록 여성; 기대 결과: 분류 D 다음 중 기존 테스트 케이스 세트에 추가하면 결정 테이블 커버리지를 증가시킬 테스트 케이스는?",
    "options": [
  ...
  },
  {
    ...
    "lo": "4.2.4",
    "level": "K3",
    "text": "네(4)가지 상태와 다섯(5)가지 이벤트가 포함된, 다음 상태 전이 테이블로 모델링되는 호텔 객실 예약 시스템에 상태 전이 테스트를 적용하고 있다: 모든 테스트 케이스가 ‘요청 중’ 상태에서 시작한다고 가정할 때, 이벤트의 순서대로 표시된 다음 테스트 케이스 중 가장 높은 유효 전이 커버리지를 달성하는 것은?",
    "options": [
  ...
  },
  {
    ...
    "lo": "4.3.1",
    "level": "K2",
    "text": "프로그램 P에 대한 테스트 스위트 S는 100% 구문 커버리지를 달성한다. 이 스위트는 세 개의 테스트 케이스로 구성돼 있고, 각 테스트 케이스는 50% 구문 커버리지를 달성한다. 다음 중 올바른 설명은 무엇인가?",
    "options": [
  ...
  },
  {
    ...
    "lo": "4.3.3",
    "level": "K2",
    "text": "다음 중 소프트웨어 명세가 모호하고 오래되었거나 불완전한 경우에도 화이트박스 테스팅으로 결함 식별이 가능한 이유는?",
    "options": [
  ...
  },
  {
    ...
    "lo": "4.5.1",
    "level": "K2",
    "text": "다음 중 팀이 전달해야 하는 내용에 대한 공통된 이해를 달성할 협업 사용자 스토리 작성 방법은 무엇인가?",
    "options": [
  ...
  },
  {
    ...
    "lo": "4.5.3",
    "level": "K3",
    "text": "다음 사용자 스토리를 위한 테스트 케이스 설계를 시작했다: 고객으로서 나는, 가격 범위에 의한 검색 결과를 필터링해서 내 예산 범위에 들어오는 제품을 좀 더 쉽게 찾을 수 있기를 원한다. 인수 조건: 1. 필터는 버전 3.0 이상인 모든 애플리케이션 버전에서 동작해야 한다 2. 필터는 고객이 최소 및 최대 가격을 설정할 수 있게 해야 한다 3. 고객이 가격 필터를 조정하면 검색 결과는 동적으로 업데이트되어야 한다 모든 테스트 케이스의 사전 조건은 다음과 같다: A와 B 두 가지 제품만 있다. 제품 A의 가격은 $100이고, 제품 B의 가격은 $110이다. 다음 중 이 사용자 스토리에 대한 테스트 케이스로 가장 적절한 것은?",
    "options": [
      "웹페이지에 접속해서 $90와 $100 사이의 제품을 보여주도록 필터를 설정한다. 기대 결과: 제품 A만 보여준다. 최대 가격을 $110으로 바꾼다. 기대 결과: 검색 결과에 제품 A와 B 모두 포함된다",
      "웹페이지에 접속한다. 기대 결과: 최소 가격과 최대 가격의 디폴트 값은 각각 $100와 $110이다. 제품 C를 추가하고 가격은 $120으로 설정한다. 웹페이지를 새로고침한다. 기대 결과: 디폴트 최대 가격이 $120로 변경된다",
      "웹페이지에 접속해서 $90와 $115 사이의 제품을 보여주도록 필터를 설정한다. 기대 결과: 제품 A와 B 모두 보여준다. 통화를 USD에서 EUR로 변경한다. 기대 결과: 현재 환율에 따라 필터 범위가 EUR 값으로 올바르게 변경된다",
      "엣지, 크롬, 오페라 세 가지 브라우저를 사용해 웹페이지에 접속한다. 각 브라우저에서 필터를 $90와 $110 사이로 설정한다. 기대 결과: 검색 결과에 제품 A와 B가 모두 포함되며, 결과 레이아웃은 모든 브라우저에서 동일하다"
    ],
    "answer": [
      0
    ],
  ...
  },
  {
    ...
    "lo": "5.1.4",
    "level": "K3",
    "text": "팀은 어느 소프트웨어 컴포넌트에 대한 네 개의 테스트 케이스를 한 명의 테스터가 실행하는 데 드는 시간을 추정하고자 한다. 팀은 테스트 케이스 한 개를 실행하는 데 들어간 노력에 대해 다음과 같은 측정값을 수집했다: • 가장 낙관적인 추정치: 1시간 • 가장 비관적인 추정치: 8시간 • 가장 가능성이 높은 추정치: 3시간 3점 추정 기법을 사용하는 경우, 네(4)개의 테스트 케이스를 모두 실행하는 데 필요한 시간의 최종 추정치는?",
    "options": [
      "14시간",
      "3.5시간",
      "16시간",
  ...
  },
  {
    ...
    "lo": "5.1.5",
    "level": "K3",
    "text": "다음 표는 테스트 케이스부터 요구사항까지의 추적성 행렬을 보여주고 있다. “X”는 주어진 테스트 케이스가 해당 요구사항을 커버하고 있음을 의미한다: 커버리지 기반 우선순위 지정 기법에 따라 테스트 케이스의 우선순위를 정하고자 한다. 네 개의 테스트 케이스는 모두 실행되어야 한다. 다음 중 마지막으로 실행해야 하는 테스트 케이스는?",
    "options": [
      "TC1",
      "TC2",
      "TC3",
  ...
  },
  {
    ...
    "lo": "5.2.1",
    "level": "K1",
    "text": "어떤 리스크의 리스크 수준이 $1,000이고, 리스크 발생 가능성이 50%로 추정된다면, 다음 중 리스크 영향도로 적절한 것은?",
    "options": [
      "$500",
      "$2,000",
      "$50,000",
  ...
  },
  {
    ...
    "lo": "5.4.1",
    "level": "K2",
    "text": "사용자가 소프트웨어 장애를 보고했다. 지원팀의 엔지니어가 사용자에게 장애가 관찰된 소프트웨어 버전을 문의했다. 버전 번호를 기반으로 팀은 릴리즈를 구성한 모든 파일을 재구성했다. 이를 통해 나중에 개발자가 장애를 분석해서 결함을 찾아 수정할 수 있었다. 다음 중 팀에서 위 활동을 수행할 수 있었던 이유로 적절한 것은?",
    "options": [
      "리스크 관리",
      "테스트 모니터링과 제어",
      "전체 팀 접근법",
